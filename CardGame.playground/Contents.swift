...
//    static var allRanks: [Rank] { return [ace, two, three, four, five, six, seven, eight, nine, ten, jack, queen, king] }
}
...
//    static var allSuits: [Suit] { return [hearts, diamonds, spades, clubs] }
}
...
extension HighLow {
    func play() {
        let player1Hand = deck.drawCard()
        let player2Hand = deck.drawCard()
//        print("player1: \(player1Hand), player2: \(player2Hand)")
        if let delagate = delegate {
            delagate.gameDidStart(cardGame: self)
            delagate.game(player1DidDraw: player1Hand, player2DidDraw: player2Hand)
        }
        if player1Hand < player2Hand {
            print("Player 2 wins with \(player2Hand)")
        } else if player1Hand == player2Hand {
            print("Round ends in a tie with \(player1Hand)")
        } else {
            print("Player 1 wins with \(player1Hand)")
        }
    }
}
...
extension Card: Comparable {
    static func < (lhs: Card, rhs: Card) -> Bool {
        // does not necessarily check for same suits, player 1 is dealer, same suit, player 1 wins
...
cardGame.delegate = cardGameDelegate
cardGame.play()
